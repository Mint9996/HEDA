...
@dataclass
class LLaMASemiConfig(FairseqDataclass):
...
@register_model("llama_semi", dataclass=LLaMASemiConfig)
class LLaMASemi(BaseFairseqModel):
...
class LLaMaTransformer(nn.Module):
    ...
    def forward(
        ...
        if incremental_state is not None:
            ...
            # self_attn_mask = torch.zeros([prev_output_tokens.size(1),prev_output_tokens.size(1)]).to(prev_output_tokens)
            # self_attn_mask = None
...
def llama_semi_base_architecture(args):
...
@register_model_architecture("llama_semi", "llama_semi_7b")
def llama_semi_7b(args):
    llama_semi_base_architecture(args)
