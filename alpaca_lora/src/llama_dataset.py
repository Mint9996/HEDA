...
def collate(
    ...
    bos_token = merge(
        "bos_token",
        left_pad=left_pad_source,
        pad_to_length=pad_to_length["bos_token"] if pad_to_length is not None else None,
    )
    bos_token = bos_token.index_select(0, sort_order)

    src_pos = merge(
        "src_pos",
        left_pad=left_pad_source,
        pad_to_length=pad_to_length["src_pos"] if pad_to_length is not None else None,
    )
    src_pos = src_pos.index_select(0, sort_order)
    bos_token_pos = merge(
        "bos_token_pos",
        left_pad=left_pad_source,
        pad_to_length=pad_to_length["bos_token_pos"] if pad_to_length is not None else None,
    )
    ...
    if samples[0].get("target", None) is not None:
        ...
        prev_output_tokens[:,0:1] =  bos_token

        tgt_pos = merge(
            "tgt_pos",
            left_pad=left_pad_source,
            move_eos_to_beginning=True,
            pad_to_length=pad_to_length["tgt_pos"] if pad_to_length is not None else None,
        )
...
class Weighted_Dataset(FairseqDataset):
